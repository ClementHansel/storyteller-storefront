...
import { setAccessToken, setRefreshToken, clearTokens } from '@/lib/tokenManager';
...
export async function registerCustomer(payload: RegisterPayload): Promise<AuthResponse> {
  const { data } = await zenvixClient.post<AuthResponse>('/auth/register', payload);
...
export async function refreshToken(token: string): Promise<AuthResponse> {
  const { data } = await zenvixClient.post<AuthResponse>('/auth/refresh', { refreshToken: token });
  setAccessToken(data.accessToken);
