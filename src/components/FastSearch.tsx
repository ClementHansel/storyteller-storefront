    ...
    <div ref={containerRef} className="relative w-full max-w-md">
      <div className="relative">
        ...
        <input
          ...
          className="w-full rounded-none border border-border bg-transparent py-2 pl-9 pr-8 text-xs placeholder:text-muted-foreground focus:outline-none focus:border-accent transition-colors"
        />
        {query && (
      ...
      {open && results.length > 0 && (
        <div className="absolute z-50 mt-1 w-full border border-border bg-popover shadow-xl">
          {results.map((p) => (
            <button
              key={p.id}
          ...
          <button
            ...
            className="w-full border-t border-border px-3 py-2.5 text-center text-[10px] uppercase tracking-widest text-accent hover:bg-muted transition-colors"
          >
            View all results
          </button>
        </div>
